...
pub fn get_content(text: &str, start: Position, end: Position) -> Option<String> {
    ...
    for (line_number, line) in lines.enumerate() {
      if start.line < line_number {continue;}
      if end.line > line_number {break;}
      for (char_number, char) in line.char_indices(){
        if start.column < char_number{continue;}
        if end.line == line_number && end.column > char_number {break;}
        string.push(char);
      }
    }
    Some(string)
